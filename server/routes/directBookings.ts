...
// collects both. Sending phone or email means both must be present.
function resolveContact(body: Record<string, unknown>): { phone: string | null; email: string | null } | string {
  ...
  if (!contact) return "phone and email are required";
  return splitContact(contact);
}
...
router.post("/", async (req, res) => {
  ...
  const { itemId, eventDate, customerName } = body;
